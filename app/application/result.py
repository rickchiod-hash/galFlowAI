"""Standardized Result class for use cases and adapters."""
...
class Result(dict):
    ...
    Inherits from dict for backward compatibility with code that uses .get("ok") etc.
    Also provides convenient attributes: ok, data, error.
    """
    
    def __init__(self, ok: bool, data: Any = None, error: str = None, **extras):
        # Initialize dict with the standard keys
        d = {"ok": ok}
        if ok:
            d["data"] = data
        else:
        ...
        d.update(extras)
        super().__init__(d)
        # Also set attributes for extras so that result.extra works
