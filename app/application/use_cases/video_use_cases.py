"""Use cases for video rendering operations.
...
class RenderVideoUseCase(UseCase):
...
class CheckWanGPAvailabilityUseCase(UseCase):
    """Check if WanGP is available.
    
    3-point standard:
    1. Validate (no input needed)
    2. Check WanGP availability
    3. Return availability status
    """
    
    def execute(self) -> Dict[str, Any]:
        """Execute WanGP availability check."""
        try:
            # 1. Validate input (none needed)
            # 2. Execute business logic
            adapter = WanGPAdapter()
            available = adapter.disponivel()
